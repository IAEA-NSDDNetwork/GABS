...
!                                  to mixture of Real(kind=4) and REAL*8 in
!                                  line 20.
...
!          1.6h   13-Feb-2019   TK: SCALDX routine: Uncertainty was rounded up if 
!                               d >= 1.1 (--> 2), 2.1 (--> 3), 3.1 (--> 4), etc
!          1.6i   08-Mar-2019   TK: Adopt rounding of uncertainties from 
!                               Review of Particle Physics (JPG 37 (2010) 075021
!                               Based on the three higher order digits in in the errror
!                               100: 354 --> round to 2 digits
!                               355: 949 --> round to 1 digit
!                               950:1000 --> round to 2 digits; i.e. "10"
!***********************************************************************
!                                                                      *
...
!
      ...
      DO i = 1,l
         ic = ICHAR(String(i:i))
         IF(ic > 96 .and. ic.LT.123)String(i:i) = CHAR(ic-32)
      END DO
!
      RETURN
...
!
      ...
      DO i = 1,l
         ic = ICHAR(String(i:i))
         IF(ic > 64 .and. ic.LT.91)String(i:i) = CHAR(ic+32)
      END DO
!
      RETURN
...
!
      ...
      DO i = 1,l
         ic = ICHAR(String(i:i))
         IF(ic > 96 .and. ic.LT.123) UPCASE(i:i) = CHAR(ic-32)
      END DO
!
      RETURN
      END FUNCTION UPCASE
...
!
      ...
      DO i = 1,l
         ic = ICHAR(String(i:i))
         IF(ic > 64 .and. ic.LT.91) LOCASE(i:i) = CHAR(ic+32)
      END DO
!
      RETURN
      END FUNCTION LOCASE
...
!
            IF(String(ilen-jlen+1:ilen) == Delim) THEN
               str = String
               ilen = ilen - jlen
            ELSE
...
!
         ...
         IF(ipath == 2)CALL SCOUNT(Outstr,Nstr)
      END IF
!
      RETURN
...
!
      DO WHILE (cline /= ' ')
         ...
         Npar = Npar + 1
         IF(isw == 0) isw = ilent + 1
         IF(isw > 0) Carray(npar) = cline(1:isw-1)
         cline = cline(isw+1:)
      END DO
!
!+++MDC+++
...
!
      ...
      DO from = 1,LEN(String)
         IF(String(from:from) /= Achar) THEN
            IF(to /= from)String(to:to) = String(from:from)
            to = to + 1
         END IF
      END DO
      IF(to == from)RETURN
...
!
      ...
      IF(INDEXF /= 0)INDEXF = INDEXF + Pos - 1
      END FUNCTION INDEXF
!
!***********************************************************************
!
...
!
      ...
      DO s = Pos,lstr
         DO i = 1,lspn
            IF(String(s:s) == Spnstr(i:i)) GO TO 10
         END DO
         SPAN = s
         RETURN
   10 END DO
...
!        SKIP LEADING BLANKS.
!
         IF(String(RLSCN:RLSCN) == ' ') THEN
            RLSCN = RLSCN + 1
            IF(RLSCN > LEN(String))RETURN
            CYCLE
         END IF
!
!        LOOK FOR SIGN.
...
!
         ...
         IF(String(RLSCN:RLSCN) == '+') THEN
            pmsign = +1
         ELSE IF(String(RLSCN:RLSCN) == '-') THEN
            pmsign = -1
         END IF
         IF(pmsign /= 0)RLSCN = RLSCN + 1
!
...
!
            IF(RLSCN.LE.LEN(String)) THEN
               IF(String(RLSCN:RLSCN) == 'E' .or. String(RLSCN:RLSCN)      &     
     &             == 'e') THEN
                  RLSCN = RLSCN + 1
...
!        SKIP LEADING BLANKS.
!
         IF(String(DRLSCN:DRLSCN) == ' ') THEN
            DRLSCN = DRLSCN + 1
            IF(DRLSCN > LEN(String))RETURN
            CYCLE
         END IF
!
!        LOOK FOR SIGN.
...
!
         ...
         IF(String(DRLSCN:DRLSCN) == '+') THEN
            pmsign = +1
         ELSE IF(String(DRLSCN:DRLSCN) == '-') THEN
            pmsign = -1
         END IF
         IF(pmsign /= 0)DRLSCN = DRLSCN + 1
!
!        LOOK FOR INTEGER PART.
!
...
!
            IF(DRLSCN.LE.LEN(String)) THEN
               IF(String(DRLSCN:DRLSCN) == 'E' .or.                        &
     &            String(DRLSCN:DRLSCN) == 'e') THEN
                  DRLSCN = DRLSCN + 1
...
!        SKIP LEADING BLANKS.
!
         IF(String(INTSCN:INTSCN) == ' ') THEN
            INTSCN = INTSCN + 1
            IF(INTSCN > LEN(String))RETURN
            CYCLE
         END IF
!
...
!***********REMOVE THE BLANKS
         ...
         END DO
!
         IF(ipath == 2)Nstr = LEN_TRIM(String)
      END IF
      RETURN
...
!
!TK 27-Feb-2019 capture if String is blank; i.e. zero length
...
!           implementing the ERR branch (TWB. 930222)
            IF(INDEX(String(istart:lstr),'E') > 0 .or.                    &     
     &         INDEX(String(istart:lstr),'.') > 0) THEN
...
!     and return
      IF(r == 1 .and. (tx(1:1).LT.'0' .or. tx(1:1) > '9'))RETURN
!
...
!
  ...
  100 IF(Sym(1:1).GE.'0' .and. Sym(1:1).LE.'9') THEN
         IF(Sym(2:2).LT.'0' .or. Sym(2:2) > '9') THEN
            El = -1
            RETURN
         END IF
         READ(Sym,FMT='(I2)') isym
         isym = isym + 100
         IF(isym.GE.104 .and. isym.LE.NSYM .and. imesg == 1) THEN
...
!        ...FIND PROPER IPWR.
         t = 0.0D0
         IF(DABS(x) > 1.D-35)t = DLOG10(DABS(x))
...
!--      FORMAT 2:  SX ONLY (SDX INCLUDED).
         ...
         idx = dx
         i = 1
         DO WHILE (DABS((dx-FLOAT(idx))/dx) > 1.D-4)
            ipwr = ipwr - 1
            dx = dx*1.D1
            idx = INT(dx+0.9)
            i = i + 1
...
!           of significant digits
            IF(Lendx.LT.0 .and. LENSTR(temp) /= -Lendx) THEN
...
!
      IVRFLW = .FALSE.
      xx = X*(10.0D+0**(-Ipwr))
      IF(xx.LT.-(2.D+0**31) .or. xx > ((2.D+0**31)-1.D0))IVRFLW = .TRUE.
      RETURN
      END FUNCTION IVRFLW
!
!***********************************************************************
...
!     Local variables
!
      INTEGER(KIND=4) :: i,iptr,jpwr,lente2,lentem,neg
      INTEGER(KIND=4),EXTERNAL :: LENSTR
...
! TK 26-Mar-2019 Last check if would not fit into Sx
            If (lenTem + iptr + 2+ neg > LenX) Go To 10
...
! 13-Feb-2019 TK: Uncertainty was rounded up if d >= 1.1 (--> 2), 2.1 (--> 3), 3.1 (--> 4), etc
!     Idx = INT(d+0.9)
      Idx = INT(d+0.5)
!
!--   RETURN TO CALLING ROUTINE.
...
!
      IF((MOD(Ix,10) == 0) .and. (MOD(Idx,10) == 0)) THEN
         Ix = Ix/10
         Idx = Idx/10
         Ipwr = Ipwr + 1
      END IF
...
!
      ...
      DO i = 1,LEN(Str)
         ...
         IF(chr /= ' ') THEN
            IF(start) THEN
               IF(ICHAR(chr).LE.ICHAR('9') .and. ICHAR(chr).GE.ICHAR('0')) start = .FALSE.
               CYCLE
            END IF
            IF(.NOT.start) THEN
               IF(chr == 'E') THEN
...
!        Too short or too long a string
         IF(strlen.LT.1 .or. strlen > 5) RETURN
!        Too short or too long a string or hyphen in wrong position
      ELSE IF(strlen .LT. 3 .or. strlen > 9 .or. hyppos .LT. 2 .or. hyppos > 4)  THEN
         RETURN
      END IF
!
      CALL UPSTR(wrkstr)
...
!        Input not in nuclear reaction form
         IF(hyppos == 0) THEN
            IF(strlen == 5) THEN
               ...
               IF(Rstr(1:1) == '0') CALL ADDSTR(Rstr,1,'1')
               Ierr = 0
!              Pure integer - must be Z and natural
            ELSE IF(TYPSTR(wrkstr(1:strlen)) == 1) THEN
               IF(strlen > 3) RETURN
            ...
            ELSE
               i = strlen
               DO WHILE (wrkstr(i:i).GE.'A' .and. wrkstr(i:i).LE.'Z' .and.  i > 0)
                  i = i - 1
               END DO
               IF(i == strlen .or. i > 3) THEN
...
!           Input looks like a reaction - check it
            i = hyppos - 1
            IF(TYPSTR(wrkstr(1:i)) /= 1) THEN
               IF(i == 0 .or. i > 2 .or. TYPSTR(wrkstr(1:i)) /= 2) RETURN
            END IF
            IF((strlen-hyppos) == 0 .or. (strlen-hyppos) > 5) RETURN
            i = hyppos + 1
            DO WHILE (i.LT.strlen .and. TYPSTR(wrkstr(i:i)) == 1)
               i = i + 1
            END DO
            IF(i.LE.strlen .and. TYPSTR(wrkstr(i:i)) /= 1) THEN
               IF(wrkstr(i:i) /= 'M' .and. wrkstr(i:i) /= 'G') RETURN
               i = i + 1
               IF(i.LE.strlen) THEN
                  IF(wrkstr(i-1:i-1) == 'G' .or. TYPSTR(wrkstr(i:i))      &       
     &                /= 1 .or. i.LT.strlen) RETURN
               END IF
            END IF
            i = hyppos - 1
            IF(TYPSTR(wrkstr(1:i)) == 2) THEN
               Rstr = wrkstr
            ELSE
...
!           Strip off metastable info if necessary
            IF(wrkstr(strlen:strlen) == 'G' .or. wrkstr(strlen:strlen)    &    
     &          == 'M') THEN
               strlen = strlen - 1
            ELSE IF(wrkstr(strlen-1:strlen-1) == 'M') THEN
               strlen = strlen - 2
            END IF
!           Non-numeric mass - error
            IF(TYPSTR(wrkstr(hyppos+1:strlen)) /= 1) THEN
               Ierr = 1
               RETURN
            END IF
!           Get the symbol
            sym = wrkstr(1:hyppos-1)
            IF(TYPSTR(sym) /= 2) THEN
!              Symbol must be alpha or numeric - error
               IF(TYPSTR(sym) /= 1) THEN
                  Ierr = 1
                  RETURN
               END IF
...
!              Only two alpha characters allowed - error
            ELSE IF(LEN_TRIM(sym) > 2) THEN
               Ierr = 1
               RETURN
            END IF
            IF(wrkstr(hyppos+1:strlen) == '0') THEN
!              Mass of zero - natural
         ...
         ELSE
!           Pure integer - must be Z and natural
            IF(TYPSTR(wrkstr(1:strlen)) == 1) THEN
               IF(strlen > 3) THEN
...
!              Input looks like structure - Check it
            ELSE IF(TYPSTR(wrkstr(1:strlen)) /= 2) THEN
               i = 1
               DO WHILE (i.LT.strlen .and. TYPSTR(wrkstr(i:i)) == 1)
                  i = i + 1
               END DO
               IF(TYPSTR(wrkstr(i:i)) /= 1 .and. TYPSTR(wrkstr(i:i)) /= 2)&      
     &            THEN
                  Ierr = 1
                  RETURN
               END IF
               IF(i == strlen .and. TYPSTR(wrkstr(i:i)) /= 2) THEN
                  IF(strlen /= 5) THEN
                     Ierr = 1
                     RETURN
                  END IF
                  ...
                  IF(sym == ' ') THEN
                     ...
                     RETURN
                  END IF
               ELSE IF(TYPSTR(wrkstr(i:strlen)) /= 2 .or.   LEN_TRIM(wrkstr(i:strlen)) > 2) THEN
                  Ierr = 1
                  RETURN
               END IF
            END IF
...
!
      ...
      DO n = 1, Keys(1)
         ioff = 4*(n-1)
         IF(Keys(ioff+2) == 3) THEN
            mkey = mkey + 12
         ELSE IF(Keys(ioff+2) == 2) THEN
            mkey = mkey + 8
         ELSE IF(Keys(ioff+2) == 1) THEN
...
!
      ...
      END IF
!
   20 IF(Iacc == 1) REWIND(UNIT=iunit)
!
      RETURN
...
!              -- We have to keep percolating.
               IF(winner(1:keysiz) > bufkey(tree(i))(1:keysiz)) THEN
...
!              -- Since we're already winner, no need to swap.
            ELSE IF(nxtrun(win) .or. winner(1:keysiz) > bufkey(tree(i))  &       
     &              (1:keysiz)) THEN
...
!     -- i is used here to tell us if there are any more data records.
      DO nbuf = 1, Mbuf
         IF(bufkey(nbuf)(1:keysiz) /= flag(1:keysiz)) THEN
            nxtrun(nbuf) = .FALSE.
...
!        -- to be merged.
         ...
         REWIND(UNIT=filout)
   55    IF(merger(0)(1:keysiz) == flag(1:keysiz)) THEN
...
!           If merger(1) is flag then it will come back and go to mrgret
            ...
            DO WHILE (merger(0)(1:keysiz) /= flag(1:keysiz))
...
!           copy
         ELSE IF(merger(1)(1:keysiz) == flag(1:keysiz)) THEN
...
!        -- Only 1 run, copy it.
         ...
         CALL SRTKEYS(Keys,merger(0)(1:recsiz),mekey(0)(1:keysiz))
         IF(mekey(0)(1:keysiz) == flag(1:keysiz)) GO TO 80
         DO WHILE (merger(0)(1:keysiz) /= flag(1:keysiz))
            CALL SORT_WRITE(filout,iunit,iacc,iform,nrio,filin,         &      
     &                merger(0)(1:recsiz),iend)
            IF(iend > 0) GO TO 90
...
!        -- Two runs, merge.
         ...
         REWIND(UNIT=filin+1)
         READ(UNIT=filin+1,END=90) merger(1)(1:recsiz)
   70    IF(merger(0)(1:keysiz) == flag(1:keysiz)) THEN
!           -- End of run from input file 0.
!           -- Copy rest of run from file 1.
            filin = filin + 1
            merger(0)(1:recsiz) = merger(1)(1:recsiz)
...
!
      ...
      DO i = 1, numkeys
         ...
         ELSE IF(styp == 2) THEN
            ival = strint
            IF(adcd == 1) ival = -ival
            IF(ival > 99999999) ival = 99999999
         ...
         ELSE IF(styp == 3) THEN
            val = streal
            IF(adcd == 1) val = -val
            IF(val > 99999999.999) val = 99999999.999
